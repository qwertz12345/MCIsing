...
\begin{abstract}
In dieser Simulation wird das Verhalten von Magnetismus in einem zweidimensionalen Atomgitter untersucht. Die Atome besitzen dabei einen Spin mit zwei entgegengesetzten Zuständen. Die verwendete Methode ist der Metropolisalgorithmus, wobei nur die Wechselwirkung zwischen benachbarten Gitterpunkten berücksichtigt wird. \\
In verschiedenen physikalischen Grössen beobachtet man einen Phasenübergang bei einer bestimmten kritischen Temperatur. 
\end{abstract}
...
\\
Einige interessante Spin-Konfigurationen bei verschiedenen Temperaturen werden in den Abbildungen (\ref{fig:ds}),(\ref{fig:dsas}) für J=1 und J=-1 dargestellt. 
\\
\\
%\section{J=1}
...
\\
Ohne externes Magnetfeld erwartet man für die Messgrössen die gleichen Ergebnisse wie für J=1. In dieser Simulation weicht aber besonders die Suszeptibilität für niedrige Temperaturen vom Fall J=1 ab. 
%Dies kann man so erklären: Für J=-1 und niedrigen Temperaturen ordnen sich die Spins in einem Schachbrettmuster an, um die Energie zu minimieren (siehe Abbildung (\ref{fig:dsas})). Daraus folgt: $\left\langle M\right\rangle = 0$. Für $J=1$ ist es dagegen energetisch günstig, wenn alle Spins in die selbe Richtung zeigen. Daher: $\left\langle M\right\rangle \neq 0$. Somit sind die Werte der Suszeptibilitäten verschieden.
